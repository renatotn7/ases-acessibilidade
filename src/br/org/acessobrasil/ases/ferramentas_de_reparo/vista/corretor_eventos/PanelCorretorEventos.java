...
import br.org.acessobrasil.ases.ferramentas_de_reparo.controle.ControleCorretorEventos;
 ...
 * 
 */
public class PanelCorretorEventos extends JPanel implements ActionListener, MouseListener {
	/**
	 * extends JFrame
	 */
	private static final long serialVersionUID = -638404432108855839L;

	G_TextAreaSourceCode textAreaSourceCode = new G_TextAreaSourceCode();
	...
	/**
	 * Texto onde é digitado o evento em JS
	 */
	private JTextField texto;

	private JPanel panelCorretor;

	private JButton btnAplicar, reverter, btn_cancelar;

	private ControleCorretorEventos controle;
	...
	PanelCorretorEventos painelOriginal;
	 ...
	 * @param frameSilvinha
	 */
	public PanelCorretorEventos(FrameSilvinha frameSilvinha, String codHtml) {
		if (codHtml == null) {
			codHtml = "";
		}
	...
	private void reavalia(String codigo) {
		...
		// boxCode.coloreSource();
		controle = new ControleCorretorEventos();
	...
	/*
	 * private void salvarComo() { if (arquivo == null) { arquivo = new
	 * G_File(""); } String cod = ""; cod = boxCode.getText(); String
	 * extensoes[] = { ".html", ".htm" }; boolean salvo =
	 * arquivo.openDialogSaveAs(cod, extensoes); if (salvo) { if (arquivo !=
	 * null && arquivo.getFile() != null) { if (arquivo.getFile().getName() !=
	 * null) { frameSilvinha.setTitle(arquivo.getFile().getName() + " - " +
	 * XHTMLPanel.TITULO); } boxCode.coloreSource(); avalia(); } } }
	 */
	...
	public void setPanelOriginal(PanelCorretorEventos painelOriginal) {
	...
	private void avaliaArq(G_File temp) {
		...
		frameSilvinha.setTitle(arquivo.getFile().getName() + " - " + XHTML_Panel.TIT_CORR_EVT);
