...
public class Geral {
	...
	/**
	 * Conteúdo HTML da página a ser avaliada.
	 */
	...
	/**
	 * Adquiri uma tag errada e a concatena dentro de um mapa de erros. Se já
	...
	private void setErrados(final ArmazenaErroOuAvisoAntigo erros) {
		...
		if (this.erradosMap.containsKey(nome)) {// 
			...
			erradoAL = new ArrayList<ArmazenaErroOuAvisoAntigo>();// senão cria uma
			// nova lista
	...
	private void avaliar(ArrayList<ArmazenaErroOuAvisoAntigo> erros) {
		...
		for (Iterator<ArmazenaErroOuAvisoAntigo> iteracao = erros.iterator(); iteracao
			...
			// Esses erros não existem, tentar otimizar depois
			// }else{
			...
			/*
			 * Se erro deve ser passado pela avaliação de tag, senão agrupe
	...
	/**
	 * Avaliador se há uma tag especifica dentro do arquivo, havendo ela
	 * distingue que a tag não está errada, exemplo script necessita de um
	 * noscript dentro do documento. Para formar toda lista de tags especificas
	 * ele busca em banco as que são obrigatórias.
	 * 
	 * @throws SQLException
	 */
	private void avaliaTagObrigatoria() throws SQLException {
		/*
		 * busca as tags obrigatórias do documento.
		 */
		...
		for (String element : this.erradosMap.keySet()) {
			/*
			 * Verifica se a existencia da tag dentro do documento. Não
	...
	private void loopingChildren(Node documento) throws IOException,
		...
		if (documento != null) {
			...
			/*
			 * Pesquisa se é uma tag especial de parâmetro.
			 */
			if (nomeTag != null
				...
				/*
				 * Tag de linha que contém o número da linha e a configuração de
				 * identação.
				 */
				...
				/*
				 * tag mãe para todas as outras.
				 */
				if (TokenNucleo.MAE.equals(nomeTag)) {
					NodeList children = documento.getChildNodes();
					for (int i = 0; i < children.getLength(); i++) {
						// Começa a fazer um loop para dentro do documento.
						loopingChildren(children.item(i));
					}
				}
			}
			/*
			 * Se a tag for Tag HTML do Conteúdo do Link.
			 */
			else {
				...
				/*
				 * Se houver um doctype ele é atribuído a lista de tags, para
				...
				/*
				 * Reescreve o nó para que se possa guardar ela montada.
				 */
				...
				/*
				 * Nó de abertura, ele apenas prossegue a pesquisa com os seus
				 * filhos.
				 */
				...
				/*
				 * Nó normal, que deve ser avaliado.
				 */
				else if (type == Node.ELEMENT_NODE) {

					/*
					 * Guarda o nome da tag para que depois ela possa ser feito
					 * o teste se há uma tag especifica no documento.
					 */
					...
					if (children != null) {
						...
						for (int pos = 0; pos < len; pos++) {
							try {
								/*
								 * Para cada filho é feito uma nova consulta.
								 */
	...
	private ArrayList<ArmazenaErroOuAvisoAntigo> mains(final String link,
		...
		/*
		 * Uma String que irá conter o XML Parametrizado das Tags existentes no
		 * HTML do link passado por Parâmetro.
		 */
		String code = null;
		if (this.conteudo == null || "".equals(this.conteudo)) {
			// JOptionPane.showMessageDialog(null, "code é:");
			// Também não passa por aqui...
			code = new Transformar().tagDefineLinha(link);
		} else {
			// JOptionPane.showMessageDialog(null, "code2 é:");
		...
		if (bidimensionaTag) {
			/*
			 * Lê o XML parametrizado e retorna uma Matriz contendo as Tags do
			 * HTML, de acordo com a linha e coluna parametrizada no método
		...
		try {
			...
			log.error("Erro na Avaliação do Link: " + link, em);
		}

		// VERIFICA A EXISTENCIA DAS TAGS OBRIGATÓRIAS
