...
public final class Validar {
     ...
     * Os Eventos são do idnome 21 a 26
     * ONCLICK'
     * ONMOUSEDOWN'
     * ONMOUSEMOVE'
     * ONMOUSEOUT'
    ...
    /**
     * Responsável pela comunicação com as outras classes
     * ele popula algumas variáveis globais e também repassa o
     * trabalho para outro método.
     * @param node Nó a ser avaliado.
     * @param ConexaoBanco Conexao com o banco de dados.
     * @param tpAvaliacao tipo de avaliação.
     * @return a lista de erros da tag.
     * @throws SQLException Erro no banco de dados ou na instrução SQL.
     */
    ...
    /**
     * Código para avaliação da tag, ela pega um conjunto
     * de tags e as submete a uma regra especifica. <br/> Existem 11
     * avaliações sendo a 8 restrita então na contagem serão 12 menos a 8.
     * <ol>
     * <li>BUSCA TAG – Busca uma tag por causa dessa tag.(script x noscript); </li>
     * <li>EXIST – A tag deve existir; </li>
     * <li>HARD CODE – Só pode ser implementado com código estruturado; </li>
     * <li>HAVER – Se houver a tag ela deve ser avaliada na sua formação; </li>
     * <li>NEXIST – A tag não pode existir; </li>
     * <li>REPET – se houver uma repetição de caracteres(Regra de imagens ASCII); </li>
     * <li>BUSCA ATR – busca erro no atributo; </li>
     * <li>TAG INTERNA – tag que deve ser interna; </li>
     * <li>ATRN – atributo não pode existir; </li>
     * <li>OBRI – Eventos de Mouse que são obrigados a ter evento de teclado;</li>
     * <li>ATRI – Atributo não precisa existir, mas se existir ele deve ser avaliado.</li>
     * </ol>
     *
     * @param regras regra na qual a tag será avaliada.
    ...
    /**
     * 
     * M&eacute;todo responsável por listar os atributos e passar a
     * avaliação para outro método.
    ...
    throws SQLException, Exception {
        ...
        //retornará para o validar(Node) que retornará para o Geral
        return motorRegra(this.buscaRegras(name));
    }

    /**
     * Método que extraí conteúdo de uma tag HTML, colocando-o como valor
    ...
    /**
     * Construtor de uma instrução sql para ser passada ao motor
     * de avaliação.
     * @param dep Id do ponto de verificação que será usado na dependência.
    ...
    /**
     * Motor que gerencia a forma de avaliação da tag, ele forma
     * as regras para a tag, verifica hierarquias e dependências.
     * @param sql Expressão sql que irá buscar as regras para com a tag.
    ...
    throws SQLException, Exception {
        ...
        /*
         * Erros que serão avaliados em Hierarquia
         * após terminar todas as regras.
         */
        ...
        for (Regra regraTag : listaRegras) {
            ...
            if (tempVal != null) {
                ...
                /*
                 * tag tem dependência de avaliação.
                 */
                if (regraTag.getDep() != 0) {
                    /*
                     * tag correta em relação a dependência.
                     */
                ...
                /*
                 * tag é colocada na lista de erros.
                 */
                if (testeErrada) {
                    regras.add(tempVal);
                }
        ...
        for (int x = 0; x < regHieraq.size(); x++) {
            ...
            for (int y = x + 1; y < regHieraq.size(); y++) {
                ...
                //hierarquizados, array e produção.
    ...
    /**
     * Retorna as regras de uma tag específica
     * @param tag
     * @return
     * @throws Exception
     */
