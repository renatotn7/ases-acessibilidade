...
public class AutenticadorDAO {
	...
	public AutenticadorDAO() throws SQLException, ClassNotFoundException {
		
		con = new ConexaoAutenticador().getCon();
	}

    public boolean autenticaUsuario(Login login) {
    	try {
    		String qry = "SELECT * FROM USERS WHERE " +
    		"login = '" + login.getUser() + "' and " +
    		"pass = '" + login.getPass() + "';";
