...
public abstract class SuperRegrasHardCoded {
	...
	public boolean verificaTamFixoAtributoWidhtHeight(String tagInteira) {
		...
		if (w.equals("") && h.equals("")) {
			// não possui width nem height então ok
			return false;
		}
	...
	/**
	 * Verifica se a extensão é .gif O ideal no futuro seria baixar a imagem e
		...
		while (matcher.find()) {
			...
			if (objects == 0) {
				...
				if (fim >= ini) {
					...
					//System.out.print("conteudo de texto do objeto='" + conteudo + "'\n");
					if (conteudo.equals("")) {
						return false;
					} else {
						return true;
	...
	public boolean verificaLabelTag(String codHTML, String tag) {
		...
		// Se for textarea ou select o type é igual a ""
		if (tipo.equals("text") || tipo.equals("password") || tipo.equals("") || tipo.equals("checkbox") || tipo.equals("radio") || tipo.equals("file")) {
	...
	public boolean verificaLabel(String codHTML, String idField) {
		//return regexFound("<label(\\s.*?|\\s+)for=\"" + idField.toLowerCase() + "\".*?>", codHTML.toLowerCase());
	...
	public boolean verificaTextoPredefinidoInput(String tagInput) {
		...
		if (!tipo.equals("text") && !tipo.equals("password") && !tipo.equals("")) {
			// Não é para verificar, a tag está ok
			return true;
		}
		String value = this.getAtributo(tagInput, "value").toLowerCase();
		if(value.equals("")){
			//value em branco, não passou no teste
			return false;
		}
		return true;
	}
	 ...
	 * 
	 * @param codHTML
	 * @return a linha do erro ou 0 (zero) caso não tenha erros
	 */
	public int verificaTextoPredefinidoBKP(String codHTML) {
		...
		while (matcher.find()) {
			...
			if (verificar) {
				// é para verificar se possui conteudo ou nao
	...
	/**
	 * Verifica a hierarquia de h1 até h6
	 * 
	 * @param codHTML
	 * @return a linha do erro ou 0 (zero) caso não tenha erros
	 */
	...
	/**
	 * Verifica se a tag tem algum parametro indicando tamanho fixo Hoje não vai
	 * até um arquivo CSS separado
	 * 
	 * @param tag
	...
	public boolean verificaTamFixoEmAtributoStyle(String tag) {
		...
		if (!matcher.find()) {
			// Não possui estilo, então está ok
			return true;
		}
		...
		/*
		Pattern widthPx = Pattern.compile("(width|height):\\s*?.*?(px|pt|em|%|ex|pc|mm|cm|in).*?;*", Pattern.DOTALL);
		matcher = widthPx.matcher(estilo);
		if (matcher.find()) {
			
			// possui px ou pt
			String unidade = matcher.group(2);
			if (!unidade.equals("%")) {
				// System.out.print("Estilo com largura fixa:
				// '"+matcher.group()+"'\n");
				return false;
			}
		}
	...
	public String getAtributo(String tagInteira, String att) {
		...
		/*
		 * Testar quando não possui aspas ou aspas simples
		 */
		...
		for(int i=0;i<par.length;i++){
			if(estado==0){
				if(att.equals(par[i])){
					//verificar se o próximo é um sinal de igual
					estado=1;
				}else{
					if(par[i].indexOf("=")!=-1){
