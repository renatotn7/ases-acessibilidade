 ...
 *
 */
public class PainelTabbedScript<PAINELFERR extends PainelScript> extends
		SuperPainelCentral implements ChangeListener,ActionListener {
	/**
	 * 
	 */
	...
	public void init(FrameSilvinha frameSilvinha) {
		...
		this.frameSilvinha.setTitle(GERAL.TIT_CONT_ALT_TAG_SCRIPT);
		editNaoEdit = new JTabbedPane();
		this.setBackground(frameSilvinha.corDefault);
		editNaoEdit.addChangeListener(this);
	}

	public PainelTabbedScript(FrameSilvinha frameSilvinha) {
		super(new BorderLayout());
		
		ferrNaoEditavel = (PAINELFERR) new PainelScript(TxtBuffer
				.getContentOriginal(), PainelScript.CONTEUDO, frameSilvinha);
		
		ferrEditavel = (PAINELFERR) new PainelScript(TxtBuffer.getContent(),
				PainelScript.CONTEUDO, frameSilvinha);
		
		init(frameSilvinha);
		adicionaPaineis(ferrEditavel, ferrNaoEditavel);
		frameSilvinha.setTitle(GERAL.TIT_CONT_ALT_TAG_SCRIPT);
	}

	public PainelTabbedScript(String string, int conteudoOuArquivo,
			FrameSilvinha frameSilvinha) {
		super(new BorderLayout());
		...
		ferrEditavel = (PAINELFERR) new PainelScript(string,
				PainelScript.CONTEUDO, frameSilvinha);
